"""Pydantic models for MCPyGhidra.

This module contains only pydantic BaseModel subclasses and the Literal type
aliases they depend on.  It has NO Ghidra/Java imports, so it can be imported
in any Python environment (tests, tooling, etc.) without a running Ghidra JVM.
"""
...
class ApplicationInfo(BaseModel):
    ...
    name: str = Field(description="Application name (e.g., 'Ghidra')")
...
class TypeSummary(BaseModel):
    """Summary information for a type in the type browser."""

    name: str = Field(description="Short name (e.g., 'istream')")
    full_path: str = Field(
        description="Full path (e.g., 'std::istream' or '/BuiltInTypes/int')"
    )
...
class MemberInfo(BaseModel):
    """Information about a struct/union member."""
...
class TypeDetails(BaseModel):
    """Detailed information about a specific type."""
...
class FieldAdditionResult(BaseModel):
    ...
    error: str | None = Field(
        default=None, description='Error message on failure, null on success'
    )
...
class VarUpdate(BaseModel):
    """Result for a single variable rename/retype operation."""

    var: str = Field(description='Original variable name')
...
class VarUpdateReport(BaseModel):
    """Structured result for update_vars — one report per call."""

    function: str = Field(description='Function name passed to update_vars')
    addr: str | None = Field(
        default=None,
        description='Resolved function entry point (hex), null on function-level failure',
    )
    results: list[VarUpdate] = Field(
        default_factory=list,
        description='Per-variable results; empty on function-level failure',
    )
    error: str | None = Field(
        default=None,
        description='Function-level error (not-found / no-vars); null on success',
    )
