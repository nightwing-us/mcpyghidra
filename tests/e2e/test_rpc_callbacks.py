...
Verifies the full round-trip:
  ...
  4. Server injects callback functions into the pyghidra scripting environment
...
# Generous timeout for Ghidra operations (analysis + script execution)
MCP_CALL_TIMEOUT = 90
...
async def _call_with_rpc(
    ...
    """Connect with RpcClientSession, run a pyghidra snippet, return result dict.

    Args:
        server_status: The headless_server fixture dict.
        code:          Python code to execute via the pyghidra tool.
    ...
    async with streamablehttp_client(url) as (read, write, _):
        async with RpcClientSession(read, write) as session:
            ...
            with anyio.fail_after(MCP_CALL_TIMEOUT):
                result = await session.call_tool('pyghidra', {'code': code, 'reset': True})
    ...
    if result.isError:
        ...
        raise AssertionError(
            f"pyghidra tool returned error: {' '.join(error_texts)}"
        )
...
async def _call_standard(server_status: dict, code: str) -> dict:
    """Connect with a standard (unmodified) ClientSession, run a pyghidra snippet.
