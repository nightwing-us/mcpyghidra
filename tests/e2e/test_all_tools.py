...
re-launching Ghidra for each test class. Addresses are resolved dynamically
...
class TestCursorTool:
    """Tool: cursor — no args; falls back to entry point in headless mode."""

    def test_cursor_returns_location_in_headless(self, headless_server):
        """cursor() falls back to entry point in headless mode (no GUI cursor)."""
        result = mcp_call_raw(headless_server, 'cursor', {})
        # In headless mode cursor degrades gracefully to the entry point address.
        assert not result.isError, (
            f"Expected cursor to succeed in headless mode (falling back to entry point), "
            f"but got error: {' '.join(item.text for item in result.content if hasattr(item, 'text'))}"
        )
        # The result should contain an address field
        content_text = ' '.join(item.text for item in result.content if hasattr(item, 'text'))
        assert '0x' in content_text.lower() or 'addr' in content_text.lower(), (
            f"Expected cursor result to contain an address, got: {content_text[:200]}"
        )
...
class TestFuncsTool:
    ...
    def test_funcs_by_name(self, headless_server):
    ...
    def test_funcs_multiple(self, headless_server):
    ...
    def test_funcs_by_address(self, headless_server, binary_addresses):
    ...
    def test_funcs_unknown_name_returns_error_field(self, headless_server):
...
class TestElicitationFallback:
    ...
    symbol table (like check_password).
    """
...
class TestUpdateVarsTool:
    ...
    def test_update_vars_finds_and_renames_variable(self, fresh_headless_server):
        ...
        # Extract a local variable name (pattern: local_NN or param_N)
        var_match = re.search(r'\b(local_[0-9a-f]+|param_\d+)\b', decomp)
...
class TestSetPrototypeTool:
    ...
    def test_set_prototype_roundtrip(self, fresh_headless_server, binary_addresses):
        ...
        # Step 3: verify signature changed
...
class TestPatchTool:
    ...
    def test_patch_nop_and_restore(self, fresh_headless_server, binary_addresses):
        ...
        # Step 1: read original byte via disasm (1 instruction)
        orig_disasm = mcp_call(fresh_headless_server, 'disasm', {
            'items': [{'addr': main_addr, 'count': 1}],
        })

        # Extract original hex bytes from disasm output (format: "addr: XX XX XX  MNEM")
        orig_byte_match = re.search(r'[0-9a-fA-F]{6,}:\s+([0-9a-fA-F]{2})', orig_disasm)
        # We'll use a safe approach: if we can't parse, just NOP and then NOP back
...
# ---------------------------------------------------------------------------
# Tools 15 + 16: begin_trans / end_trans
# ---------------------------------------------------------------------------
...
class TestTypesTool:
    """Tool: list(entry_type='type') — types are now accessed via the list tool."""

    def test_types_lists_builtin_types(self, headless_server):
        """list(entry_type='type') returns a non-empty ListResult including primitive types."""
        result_text = mcp_call(headless_server, 'list', {
            'entry_type': 'type',
            'offset': 0,
            'limit': 50,
        })
        ...
        assert len(items) > 0
        # Common primitive type names expected in Ghidra type system
        names = [item.get('name', '') for item in items]
        assert any(prim in n for prim in ('int', 'char', 'void', 'byte', 'short') for n in names)

    def test_types_with_match_filter(self, headless_server):
        """list(entry_type='type', match_filter='int') returns only int-related types."""
        result_text = mcp_call(headless_server, 'list', {
            'entry_type': 'type',
            'match_filter': 'int',
            'offset': 0,
        ...
        parsed = parse_json_response(result_text)
        assert parsed is not None, f"Expected JSON response, got: {result_text[:200]}"
        items = parsed.get('items', [])
        assert len(items) > 0, "Filter 'int' returned no items"
        # Every returned item's name or path must contain the filter term
        for item in items:
            name = item.get('name', '') or ''
            path = item.get('path', '') or ''
            assert 'int' in name.lower() or 'int' in path.lower(), (
                f"Item name={name!r} path={path!r} does not contain 'int' — filter not applied"
            )
    ...
    def test_types_pagination(self, headless_server):
        ...
        # Pages should have different content (unless fewer than 5 types total)
        parsed0 = parse_json_response(result_page0)
        parsed1 = parse_json_response(result_page1)
        if parsed0 and parsed1:
            items0 = [item.get('name') for item in parsed0.get('items', [])]
            items1 = [item.get('name') for item in parsed1.get('items', [])]
            assert items0 != items1 or len(items0) == 0
...
class TestStructTools:
    ...
    def test_create_struct_and_add_field(self, fresh_headless_server):
        ...
        result_create = mcp_call(fresh_headless_server, 'create_struct', {
            'name': struct_name,
            'size': 16,
        })
        assert struct_name in result_create or 'created' in result_create.lower() or 'exists' in result_create.lower()
    ...
    def test_create_struct_with_initial_fields(self, fresh_headless_server):
        ...
        assert struct_name in result or 'created' in result.lower() or 'exists' in result.lower()
    ...
    def test_add_field_to_nonexistent_struct_returns_error(self, fresh_headless_server):
        """add_field on a non-existent struct returns per-item error."""
        result = mcp_call(fresh_headless_server, 'add_field', {
            'items': [{
...
# ---------------------------------------------------------------------------
# Tool: pyghidra (script execution)
# ---------------------------------------------------------------------------

class TestPyghidra:
    """Tool: pyghidra — executes Python code in the pyghidra scripting environment."""

    def test_simple_eval(self, headless_server):
        """pyghidra('1+1') returns result='2' and success=True."""
        result = mcp_call(headless_server, 'pyghidra', {'code': '1+1'})
        parsed = json.loads(result)
        assert parsed['success'] is True
        assert parsed['result'] == '2'

    def test_api_access(self, headless_server):
        """pyghidra can access currentProgram via GhidraScript globals."""
        result = mcp_call(headless_server, 'pyghidra', {
    ...
    def test_stdout_capture(self, headless_server):
        """pyghidra captures print() output in stdout and output fields."""
        result = mcp_call(headless_server, 'pyghidra', {
    ...
    def test_error_returns_success_false(self, headless_server):
        """pyghidra returns success=False on exception."""
        result = mcp_call(headless_server, 'pyghidra', {'code': '1/0'})
        parsed = json.loads(result)
    ...
    def test_list_functions_via_script(self, headless_server):
        """Use pyghidra to list all function names — proves real binary inspection."""
        result = mcp_call(headless_server, 'pyghidra', {
            'code': '[f.getName() for f in currentProgram.getFunctionManager().getFunctions(True)]',
        })
        parsed = json.loads(result)
        assert parsed['success'] is True
    ...
    def test_read_memory_via_script(self, headless_server):
        """Use pyghidra to read bytes from the binary at a known function."""
        result = mcp_call(headless_server, 'pyghidra', {
            'code': (
                'func = getFunction("main")\n'
                'addr = func.getEntryPoint()\n'
                'b = getBytes(addr, 4)\n'
                '" ".join(f"{x & 0xFF:02X}" for x in b)'
            ),
        })
        parsed = json.loads(result)
        assert parsed['success'] is True
        # Should be hex bytes like "55 48 89 E5" or similar
        assert len(parsed['result']) > 0

    def test_decompile_via_script(self, headless_server):
        """Use pyghidra to decompile main via the scripting tool."""
        result = mcp_call(headless_server, 'pyghidra', {
            'code': (
                ...
                'func = getFunction("main")\n'
                'r = decomp.decompileFunction(func, 30, ConsoleTaskMonitor())\n'
                'r.getDecompiledFunction().getC()'
            ),
        })
        parsed = json.loads(result)
        assert parsed['success'] is True, f'Error: {parsed.get("error")}'
        assert 'check_password' in parsed['result']

    def test_xrefs_via_script(self, headless_server):
        """Use pyghidra to count cross-references to check_password."""
        result = mcp_call(headless_server, 'pyghidra', {
            'code': (
...
class TestFindInsns:
    ...
    def test_find_ret_sequence(self, headless_server):
        ...
        result = mcp_call(headless_server, 'find_insns', {
            'sequences': [[{'mnemonic': 'RET', 'operands': []}]],
        })
        entries = _parse_list_response(result)
        assert len(entries) > 0, f'Expected result entries, got empty list from: {result[:200]}'
        entry = entries[0]
...
class TestStructPointerType:
    ...
    def test_set_param_type_to_struct_pointer(self, fresh_headless_server):
        ...
        # Step 2: Set param_1 type to E2eTestConfig*
        result = mcp_call(fresh_headless_server, 'update_vars', {
            'function_name': 'check_password',
            'variables_to_update': {
        ...
        result = mcp_call(fresh_headless_server, 'pyghidra', {
            'code': (
                ...
                "func = getFunction('check_password')\n"
                "r = d.decompileFunction(func, 30, ConsoleTaskMonitor())\n"
                "r.getDecompiledFunction().getC()"
            ),
        })
        parsed = json.loads(result)
...
class TestLocalVarStructPointer:
    ...
    def test_set_local_var_to_struct_pointer(self, struct_test_server):
        ...
        result = mcp_call(struct_test_server, 'pyghidra', {
            'code': (
                "from ghidra.app.decompiler import DecompInterface\n"
                "from ghidra.util.task import ConsoleTaskMonitor\n"
                "d = DecompInterface()\n"
                "d.openProgram(currentProgram)\n"
                "func = getFunction('process_config')\n"
                "r = d.decompileFunction(func, 30, ConsoleTaskMonitor())\n"
                "r.getDecompiledFunction().getC()"
            ),
        })
...
class TestScriptingPersistence:
    ...
    def test_variable_persists_over_mcp(self, headless_server):
        ...
        r1 = mcp_call(headless_server, 'pyghidra', {'code': 'session_var_e2e = 123'})
        parsed1 = json.loads(r1)
        assert parsed1['success']

        r2 = mcp_call(headless_server, 'pyghidra', {'code': 'session_var_e2e'})
    ...
    def test_function_persists_over_mcp(self, headless_server):
        ...
        mcp_call(headless_server, 'pyghidra', {
            'code': 'def _e2e_greet(): return "hello_persistent"',
        })
        r = mcp_call(headless_server, 'pyghidra', {'code': '_e2e_greet()'})
        parsed = json.loads(r)
    ...
    def test_reset_over_mcp(self, headless_server):
        ...
        mcp_call(headless_server, 'pyghidra', {'code': 'mcp_var_to_clear = 456'})

        r_reset = mcp_call(headless_server, 'pyghidra', {'code': '', 'reset': True})
        parsed_reset = json.loads(r_reset)
        assert parsed_reset['success']

        r2 = mcp_call_raw(headless_server, 'pyghidra', {'code': 'mcp_var_to_clear'})
    ...
    def test_reset_preserves_ghidra_apis(self, headless_server):
        """After reset, Ghidra APIs are still accessible over MCP transport."""
        mcp_call(headless_server, 'pyghidra', {'code': '', 'reset': True})
        r = mcp_call(headless_server, 'pyghidra', {
            'code': 'currentProgram.getName()',
        })
        parsed = json.loads(r)
        assert parsed['success']
