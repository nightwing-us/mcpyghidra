...
Tests run against a headless Ghidra server loaded with crackme.elf.

All tests use the module-scoped headless_server fixture so Ghidra is
...
class TestCallgraphTool:
    ...
    def test_callgraph_callees(self, headless_server):
        ...
        # Edges should use 'from'/'to' aliases (set via model Field alias)
        edge = data['edges'][0]
        assert 'from' in edge
        assert 'to' in edge
    ...
    def test_callgraph_root_is_main(self, headless_server):
        ...
        data = json.loads(result)
        root_addr = data['root']
        # Root node should be in node list at depth 0
...
class TestServerInfoResource:
    ...
    def test_instructions_in_handshake(self, headless_server):
        ...
        assert 'MCP Server' in result.instructions or 'MCPyGhidra' in result.instructions, (
