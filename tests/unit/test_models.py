"""Unit tests for pydantic models in mcpyghidra.models.

These tests do NOT require Ghidra or any Java runtime — models.py has no
Ghidra/Java imports.
"""
...
class TestTypeSummary:
    def test_required_fields(self):
        ts = TypeSummary(
            name='MyStruct',
            full_path='/LocalTypes/MyStruct',
            type_string='MyStruct',
            kind='struct',
            size=16,
        )
        assert ts.name == 'MyStruct'
        assert ts.full_path == '/LocalTypes/MyStruct'
...
class TestBinaryContext:
    ...
    def test_construction(self):
        ...
        assert ctx.application.name == 'Ghidra'
    ...
    def test_current_location_function_optional(self):
        ctx = BinaryContext(
            ...
            analysis=AnalysisState(
                database_path='/tmp/x.gpr',
                function_count=0,
                has_debug_symbols=False,
                has_type_libraries=False,
            ),
            application=ApplicationInfo(name='Ghidra', version='11.0'),
        )
