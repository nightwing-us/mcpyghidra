"""Unit tests for mcpyghidra.rpc_callbacks.

These tests do NOT require Ghidra or any Java runtime — rpc_callbacks.py has
no Ghidra/Java imports.
...
class TestIsNameSafe:
    ...
    def test_existing_global_is_blocked(self):
        existing = {'currentProgram': object()}
        assert is_name_safe('currentProgram', existing) is False

    def test_multiple_existing_globals(self):
        existing = {'askString': object(), 'idc': object()}
        assert is_name_safe('askString', existing) is False
        assert is_name_safe('idc', existing) is False

    def test_safe_name_not_in_existing_globals(self):
        existing = {'askString': object()}
